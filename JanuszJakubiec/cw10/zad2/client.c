...
void* server_communicator_func()
{
  while(1)
  {
    ...
    if(recv(my_desc, &msg, sizeof(struct msg_to_client), MSG_DONTWAIT) > 0)
    {
...
int create_local_socket(char* path, struct msg_to_server msg)
{
  ...
  my_desc = socket(AF_UNIX, SOCK_DGRAM, 0);
  struct sockaddr_un my_struct;
  memset(&my_struct, 0, sizeof(struct sockaddr_un));
  my_struct.sun_family = AF_UNIX;
  sprintf(my_struct.sun_path, "%d", getpid());
  bind(my_desc, (struct sockaddr*)&my_struct, sizeof(struct sockaddr_un));
  sendto(my_desc, &msg, sizeof(struct msg_to_server), 0, (struct sockaddr*)&socket_struct, sizeof(struct sockaddr_un));
...
int main(int argc, char *argv[])
{
  ...
  {
    socket = create_local_socket(argv[3], msg);
  }
  else
  {
  ...
  read(my_desc, &response, sizeof(struct msg_to_client));
  server_desc = socket;
  printf("%d\n", response.number);
