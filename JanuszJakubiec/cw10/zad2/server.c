...
void send_msg_to_client(int fd, struct sockaddr addr, struct msg_to_client msg_response)
{
  sendto(fd, &msg_response, sizeof(struct msg_to_client), 0, &addr, sizeof(struct addrinfo));
}
...
void* game(void* arg)
{
  ...
  int id1 = data.id;
  struct sockaddr addr1 = clients_list[id1].addr;
  ...
  for(int i = 0; i<CLIENTS_N; i++)
  {
    if(clients_list[i].is_active == 1 && strcmp(clients_list[i].name, data.name) == 0 && i != id1)
    {
      if(clients_list[i].is_playing_the_game != -1)
      {
         ...
         send_msg_to_client(desc1, addr1, msg_response);
         pthread_mutex_unlock(&mutex);
         return NULL;
      }
      id2 = i;
      desc2 = clients_list[i].fd;
      addr2 = clients_list[i].addr;
  ...
  else
  {
    ...
    send_msg_to_client(desc2, addr2, msg_response);
  }
  while(1)
  {
...
{
  unlink(path);
  int addr = socket(AF_UNIX, SOCK_DGRAM, 0);
...
int create_global_socket(char *port)
{
  ...
  int addr = socket(AF_INET, SOCK_DGRAM, 0);
  ...
  while(1)
  {
    ...
    for(int i = 0; i<CLIENTS_N; i++)
    {
      if(clients_list[i].is_active == 1)
      {
        ...
        send_msg_to_client(clients_list[i].fd, clients_list[i].addr, msg);
      }
    }
    pthread_mutex_unlock(&mutex);
    sleep(5);
...
int add_client(struct msg_to_server msg, struct sockaddr addr, int cli_desc)
{
  int index = -1;
  for(int i = 0; i < CLIENTS_N; i++)
  {
    ...
    else
    {
      if(strcmp(clients_list[i].name, msg.name) == 0)
      {
        ...
        send_msg_to_client(cli_desc, addr, msg_response);
        return -1;
      }
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  while(1)
  {
    ...
    recvfrom(result_event.data.fd, msg, sizeof(struct msg_to_server), 0, &client, &from_length);
    if(msg->msg_type == REGISTER_CUSTOMER)
    {
      add_client(*msg, client, result_event.data.fd);
    }
